...
__export(src_exports, {
  ...
  analyzeGitRisk: () => analyzeGitRisk,
  analyzeHttpCalls: () => analyzeHttpCalls,
  analyzeNestJSFiles: () => analyzeNestJSFiles,
  analyzeReactFiles: () => analyzeReactFiles,
  analyzeUnusedExports: () => analyzeUnusedExports,
  calculateImpact: () => calculateImpact,
  detectHttpCalls: () => detectHttpCalls,
  detectMonorepo: () => detectMonorepo,
  extractImports: () => extractImports,
  generateClaudeMd: () => generateClaudeMd,
  generateGraph: () => generateGraph,
  generateHealthReport: () => generateHealthReport,
  generateHtmlGraph: () => generateHtmlGraph,
...
function calculateImpact(changedFile, root, graph) {
  const abs = import_node_path10.default.isAbsolute(changedFile) ? changedFile : import_node_path10.default.resolve(root, changedFile);
  const impacted = graph.getImpacted(abs);
  const criticalFiles = impacted.filter(({ file }) => graph.getImportedBy(file).length >= 3).map(({ file }) => import_node_path10.default.relative(root, file));
  return {
    changedFile: import_node_path10.default.relative(root, abs),
    impacted: impacted.map(({ file, depth }) => ({
      file: import_node_path10.default.relative(root, file),
      depth
    })),
